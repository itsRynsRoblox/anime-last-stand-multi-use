...
class RapidOcr {
    ...
    ; opencv4.8.0 Mat
    ocr_from_mat(mat, param := 0, allresult := false) => DllCall('RapidOcrOnnx\OcrDetectMat', 'ptr', this, 'ptr',
        mat,
    ...
    ; Image binary data
    ocr_from_binary(data, size, param := 0, allresult := false) => DllCall('RapidOcrOnnx\OcrDetectBinary', 'ptr',
        this,
...
StartRapidOcr() {
    global ocrEngine, dllPath
    try {
        dllPath := A_ScriptDir '\\Lib\\RapidOcr\\' (A_PtrSize * 8) 'bit\\RapidOcrOnnx.dll'

        if !FileExist(dllPath) {
            AddToLog("OCR DLL not found: " dllPath)
            return false
        }

        ocrEngine := RapidOcr({ models: A_ScriptDir "\\Lib\\RapidOcr\\models" }, dllPath)

        if (!IsObject(ocrEngine) || !HasMethod(ocrEngine, "ocr_from_file")) {
            AddToLog("Failed to initialize OCR object")
            return false
        }

        return true
    } catch Error as e {
        AddToLog("OCR initialization error: " e.message)
        return false
    }
}

ReadText(x1, y1, x2, y2, scale := 2, keepImage := false) {
    try {
        ...
        baseName := "OCR"
        fullPath := imageDir "\" baseName ".png"

        if (keepImage) {
            index := 1
            while FileExist(fullPath) {
                fullPath := imageDir "\" baseName "-" index ".png"
                index++
            }
        } else if FileExist(fullPath) {
            FileDelete(fullPath)
        }

        ; Capture and process screenshot
        ImagePutFile({
            screenshot: [x1, y1, width, height],
            scale: scale * 4,
            grayscale: true,
            contrast: 2.8,
            brightness: -0.1,
        ...
        param := RapidOcr.OcrParam()
        param.padding := 20
        param.maxSideLen := 2048
        param.boxScoreThresh := 0.4
        param.boxThresh := 0.2
        param.unClipRatio := 2.5
        param.doAngle := false
        param.mostAngle := false

        ocrData := ocrEngine.ocr_from_file(fullPath, param, true)

        ; Delete only if keepImage is false
        ...
        result := ""
        if IsObject(ocrData) {
            for block in ocrData {
                if block.text {
                    cleaned := Trim(RegExReplace(block.text, "[\r\n\t]+|\s+", " "))
                    if cleaned != ""
                        result .= cleaned . " "
                }
            }
